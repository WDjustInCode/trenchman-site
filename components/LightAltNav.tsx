...
export default function LightAltNav({ profile }: { profile: NavProfile | null }) {
    ...
    <nav className="absolute top-0 left-0 right-0 z-50 bg-gradient-to-b from-white via-white/70 to-transparent pb-10">
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between pt-4">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <ProfileMenu profile={profile} theme="light" variant="desktop" />
        </div>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden bg-white/95 border-t border-gold/20 px-6 py-4 flex flex-col gap-4">
          ...
          <ProfileMenu profile={profile} theme="light" variant="mobile" onNavigate={() => setOpen(false)} />
        </div>
      )}
    </nav>
  );
